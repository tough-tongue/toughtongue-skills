...
- Uses Galaxy `medium-stable` by default — Realtime pipeline, native Vertex
  voice, no TTS/STT. See [../entities/scenario/model-selection.md](../entities/scenario/model-selection.md).
...
- Then follow the live `end_session` turn rule; never call it in the same
  turn as the dismissal
```
...
| **FIRM NO**   | Poor pitch or termination triggered | Direct rejection, then follow the closing turn rule |
...
- FLOW first turn: Start with a guarded greeting (call-type table). Then STOP
  and WAIT. Do not quote a canned line.
  ...
  "strategy": {
    ...
    "conductor": {
      ...
      "messages": [
        {
          "time_seconds": 700,
          "message": "Move toward a decision — CONVINCED, NOT READY, or FIRM NO. State the close; wait for the rep's reply before end_session.",
          "end_turn": true
        }
      ]
    }
...
For a scenario where the prospect answers first, set it to `false` and put
the prospect's greeting in FLOW (directive, never quoted).

Galaxy `medium-stable` uses native Vertex voice — no TTS/STT/LLM sub-fields.
Set `appearance.voice` to choose the voice persona.
...
- [../scenario-authoring.md](../scenario-authoring.md) — realism and difficulty principles
