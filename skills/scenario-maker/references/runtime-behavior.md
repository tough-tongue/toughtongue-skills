...
**Greeting:** if `skip_auto_start` is false, the worker asks the model to
"Greet the user with your opening line." That line must exist in FLOW. There
is no `strategy.welcome_instructions` field.
...
| ------------------------------ | ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tool set                       | Full catalog (card, mcq, browser, slides, ...) | Server tools: `end_session`, `knowledge_base_search`, `collect_data`, `cold_transfer`, plus `ask_conductor` if an on-demand conductor message is set |
...
Typical practice threshold: 4000-8000 ms. "Long silence then the call dropped"
almost always means the default hung up, `end_session: true` where a nudge
was wanted, or a threshold that's too low.
...
| Field                          | Effect                      | Notes                                                             |
| ------------------------------ | --------------------------- | ----------------------------------------------------------------- |
| `skip_auto_start`              | Who speaks first            | `false` = AI greets from FLOW; `true` = user                      |
| `silence.silence_threshold`    | ms before silence action    | **Omit = 120000 then hang up.** Practice: 4000-8000 talkative     |
| `silence.end_session`          | Disconnect vs nudge         | See section 4                                                     |
| `conductor.messages[]`         | Timed directives            | See section 3. Do not send `prefix`                               |
...
`cascade`, the filler config is silently ignored. Flag it if you spot it.
Never write `cascade-01` — use `cascade`.
...
| "Long silence then call drops"                   | `strategy.silence`                               | Omitted (120s hangup), or `end_session: true` when a nudge was wanted |
| "Agent monologues at start"                      | `ai_instructions` FLOW opening                   | Directive Beat 1 + "Then STOP and wait"; `skip_auto_start: false`     |
| "Restarts its opening when interrupted"          | `ai_instructions` FLOW                           | Quoted opening — rewrite as a directive                               |
