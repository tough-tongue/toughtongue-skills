...
Rules to include in `ai_instructions` when the scenario uses Landmass
**Cascade** (`model: "cascade"` with external TTS/STT). These supplement the
scenario's core instructions — they do not replace the call flow or domain
content. Skip this file for Galaxy, Ocean, and half-cascade (realtime
Landmass + `tts_provider` only).
...
Cascade = STT → LLM → TTS (`ai_model_config.model` is `cascade`). The customer
speaks, speech-to-text transcribes it, the LLM generates a text response, and
text-to-speech reads it aloud. The LLM never hears audio directly — it sees
imperfect transcripts and produces text that will be spoken. Every rule below
exists because of this architecture. The model code is `cascade` (formerly
`cascade-01` — never write the old name).
...
Apply to ALL Hindi text: `ai_instructions` examples, FLOW Beat 1, filler
words, edge-case dialogues.
...
**Beat 2 (response-gated):** Branch on YES / NO / BUSY.
 ...
 - NO → thank, then follow the goodbye → response → `end_session` rule
...
- [../scenario-authoring.md](../scenario-authoring.md) — voice-experience principles
- [../entities/scenario/model-selection.md](../entities/scenario/model-selection.md) — Cascade selection
- [../entities/scenario/ai-instructions.md](../entities/scenario/ai-instructions.md) — core prompt structure
